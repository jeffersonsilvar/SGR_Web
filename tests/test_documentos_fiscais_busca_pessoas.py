...
def test_busca_pessoas_normaliza_cpf_cnpj_sem_mascara():
    ...
    assert "REPLACE(REPLACE(REPLACE(REPLACE(cpf_cnpj" in fonte
    assert "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(cpf_cnpj" not in fonte
    assert "_somente_digitos(termo)" in fonte
    assert 'ROTA_BUSCA_PESSOAS = "/documentos-fiscais/api/pessoas"' in fonte


def test_busca_pessoas_nao_usa_escape_backslash_incompativel_mysql56():
    fonte = (ROOT / "app_modules" / "documentos" / "busca_pessoas.py").read_text(encoding="utf-8")
    assert "'\\\\', ''" not in fonte
    assert "REPLACE(REPLACE(REPLACE(REPLACE(cpf_cnpj, '.', ''), '/', ''), '-', ''), ' ', '') LIKE %s" in fonte
