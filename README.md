...
O módulo `google_drive_storage.py` concentra a integração opcional com Google Drive, incluindo:
...
│   ├── LIMPEZA_HISTORICO_GIT.md
│   ├── PROXIMOS_PASSOS.md
│   └── SEGURANCA_E_CONFIGURACAO.md
│
├── scripts/
│   ├── google_drive_oauth_setup.py
│   └── testar_google_drive_upload.py
│
├── static/
│   ├── css/
│   ├── js/
...
| `danfse_parser.py` | Leitura e normalização de XML de NFS-e/DANFSe |
| `google_drive_storage.py` | Integração e armazenamento no Google Drive |
...
- `seed_demo.sql` é reservado a dados completamente fictícios;
...
```bash
mysql -u root -p sgr_web < database/schema.sql
```

Caso existam dados fictícios no `seed_demo.sql`:

```bash
mysql -u root -p sgr_web < database/seed_demo.sql
```
...
GOOGLE_DRIVE_ROOT_FOLDER_NAME=SGR Web
```
...
```bash
git clone https://github.com/jeffersonsilvar/SGR_Web.git
cd SGR_Web
```
...
- nunca publicar dados reais em `seed_demo.sql`;
...
```text
docs/SEGURANCA_E_CONFIGURACAO.md
docs/LIMPEZA_HISTORICO_GIT.md
```
